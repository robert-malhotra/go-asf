...
func (c *Client) Download(ctx context.Context, product Product, destPath string) error {
	if c == nil {
		return fmt.Errorf("asf: client is nil")
	}
	if destPath == "" {
	...
	if len(urls) == 0 {
		return fmt.Errorf("asf: product missing download URL")
	}
...
type featureProps struct {
	...
	PathNumber      numericValue `json:"pathNumber"`
	Orbit           numericValue `json:"orbit"`
	FrameNumber     numericValue `json:"frameNumber"`
	MD5Sum          string       `json:"md5sum"`
	PgeVersion      string       `json:"pgeVersion"`
	CenterLat       float64      `json:"centerLat"`
	CenterLon       float64      `json:"centerLon"`
	Browse          stringList   `json:"browse"`
...
func (f feature) toProduct() Product {
	start := parseTime(f.Properties.StartTime)
	stop := parseTime(f.Properties.StopTime)
	processingDate := parseTime(f.Properties.ProcessingDate)
	...
	bytes := int64(math.Round(float64(f.Properties.Bytes)))
	if sizeMB == 0 && bytes > 0 {
	...
	browse := ""
	if len(f.Properties.Browse) > 0 {
		browse = f.Properties.Browse[0]
	}
	...
	return Product{
		...
		FlightDirection: f.Properties.FlightDirection,
		Orbit:           int(math.Round(float64(f.Properties.Orbit))),
		PathNumber:      int(math.Round(float64(f.Properties.PathNumber))),
		FrameNumber:     int(math.Round(float64(f.Properties.FrameNumber))),
		GroupID:         f.Properties.GroupID,
		Sensor:          f.Properties.Sensor,
		GranuleType:     f.Properties.GranuleType,
		MD5Sum:          f.Properties.MD5Sum,
		PgeVersion:      f.Properties.PgeVersion,
		SceneName:       f.Properties.SceneName,
		FileID:          f.Properties.FileID,
		FileName:        f.Properties.FileName,
		BrowseURL:       browse,
		CenterLat:       f.Properties.CenterLat,
		CenterLon:       f.Properties.CenterLon,
		SizeMB:          sizeMB,
		Bytes:           bytes,
...
func NewSession(opts ...SessionOption) *Session {
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if len(via) >= 10 {
			return fmt.Errorf("asf: stopped after %d redirects", len(via))
		}
		prev := via[len(via)-1]
